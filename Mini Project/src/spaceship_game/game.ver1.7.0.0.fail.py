...
# 별 이미지
starImage = ['star/star1.png', 'star/star2.png']


# 게임 초기화
def initGame():
    global gamePad, clock, background, fighter, missile, explosion, cooldown_font
    pygame.init()
...
class Star:
    def __init__(self):
        self.image = pygame.image.load(random.choice(starImage))
        self.size = self.image.get_rect().size
        self.width = self.size[0]
        self.height = self.size[1]
        self.x = random.randrange(0, padWidth - self.width)
        self.y = 0
        self.speed = 1  # 별의 떨어지는 속도 (랜덤)
        self.attack_count = 0  # 별이 공격당한 횟수
        
    def move(self):
        self.y += self.speed
        # 별이 화면 아래로 벗어나면 다시 위로 올려놓기
        if self.y > padHeight:
            self.x = random.randrange(0, padWidth - self.width)
            self.y = 0
            
    # 별이 공격당한 횟수를 증가시키는 메서드
...
# 게임 루프 정의
def runGame():
    global gamepad, clock, background, fighter, missile, explosion, missile_interval, missile_direction, missile_group, bg_y1, bg_y2, cooldown_time,destroyed_rock_count
    ...
    # 운석 객체들을 저장할 리스트
    rocks = [Rock() for _ in range(25)]  # 5개의 운석 생성
    ...
    def writeScoreStar(count):
        global gamePad
        font = pygame.font.Font('NanumGothic.ttf', 20)
        text = font.render('파괴한 별 수:' + str(count), True, (255, 255, 255))
    ...
    # 별이 통과한 개수
    def writePassedStar(count):
        global gamePad
        font = pygame.font.Font('NanumGothic.ttf', 20)
        text = font.render('지나간 별 : ' + str(count), True, (250, 0, 0))
    ...
    while not onGame:
        ...
        # 운석 맞춘 점수 표시
        writeScore(shotCountRock)
        writeScoreStar(shotCountStar)
        ...
        for star in stars:
            ...
            # 별이 아래로 내려갔을때 다시 위로 배치
            if star.y + star.height > padHeight:
                star.x = random.randrange(0, padWidth - star.width)
                star.y = 0
                star.speed = 1
                starPassed += 1
                
            drawObject(star.image, star.x, star.y)
            ...
            for missile_pos in missile_group:
                missileX, missileY = missile_pos
                for star in stars:
                    if isCollision(star.x, star.y, missileX, missileY, star.width, star.height, missile.get_width(), missile.get_height()):
                        missile_group.remove(missile_pos)
                        star.increase_attack_count()  # 별의 공격 횟수 증가
                        if star.attack_count >= 2:
                            # 별이 두 번 공격당한 경우 폭발 처리
                            isShotStar = True
                            shotCountStar += 1
            ...
            if isShotStar:
                # 운석 폭발 이미지 그리기
                drawObject(explosion, star.x, star.y)
                pygame.display.update()
        ...
        # 운석과 비행기 충돌 체크
            if isCollision(x, y, rock.x, rock.y, fighterWidth, fighterHeight, rock.width, rock.height):
                ...
                # 별과 비행기 충돌 체크
            if isCollision(x, y, star.x, star.y, fighterWidth, fighterHeight, star.width, star.height):
                crash()
