...
# 1.6.3.0 목숨 이미지화
# 1.6.3.1 공격 오토
# 1.6.3.3 일정점수 달성시 클리어
# 1.6.3.4 운석 파괴시 비행기 속도 증가
...
for filename in os.listdir(folder_path):
    file_extension = os.path.splitext(
        filename)[1].lower()  # 확장자를 추출하고 그 확장자를 소문자로 변환
...
def initGame():
    ...
    life_image = pygame.image.load("object/life.png")  # 목숨 이미지
    life_image = pygame.transform.scale(life_image, (20, 20))  # 목숨 이미지 사이즈
...
def writeWin(text):
    global gamePad
    textfont = pygame.font.Font("NanumGothic.ttf", 80)
    text = textfont.render(text, True, (255, 255, 0))
    textpos = text.get_rect()
    textpos.center = (padWidth / 2, padHeight / 2)
    gamePad.blit(text, textpos)
    pygame.display.update()
...
def runGame():
    ...
    # 운석 객체들을 저장할 리스트
    rocks = [Rock() for _ in range(7)]  # 5개의 운석 생성
            ...
            if event.type in [pygame.KEYDOWN]:
                if event.key == pygame.K_LEFT:  # 비행기 왼쪽 이동
                    ...
                    if cooldown_time <= 0:
                        ...
                        # 20발 미사일이므로 중앙에서 좌우로 (10 * 20 / 2) 픽셀씩 이동
                        missileX = (x + fighterWidth / 2 - (100 * 10 / 2))
                        missileY = y - fighterHeight
                        for i in range(100):
                            missile_group.append([missileX, missileY])
