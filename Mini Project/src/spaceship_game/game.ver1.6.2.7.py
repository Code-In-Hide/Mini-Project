...
# 게임 초기화
def initGame():
    global gamePad, clock, background, fighter, missile, explosion, cooldown_font, missilesound, crashsound, rocksound
    pygame.init()
            ...
            if event.type in [pygame.KEYDOWN]:
                if event.key == pygame.K_LEFT:  # 비행기 왼쪽 이동
                    ...
                    if cooldown_time <= 0:
                        ...
                        missileX = x + fighterWidth / 2 - (10 * 20 / 2)  # 20발 미사일이므로 중앙에서 좌우로 10발씩 배치
                        missileY = y - fighterHeight
                        for i in range(20):
                            missile_group.append([missileX, missileY])
